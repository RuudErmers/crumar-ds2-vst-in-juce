...
void XMoogFilter::setCutoff(double cutoff)
{
	if (FCutoff != cutoff)
	{
		FCutoff = cutoff;
...
void XMoogFilter::setResonance(double resonance)
{
...
void XMoogFilter::setSampleRate(double value)
{
	FSampleRate = value;
	calc();
}
...
double XMoogFilter::process(double input)
{
	double result;
	// process input
	if (input>1) input = 1;
